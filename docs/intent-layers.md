...
!!! quote "Trivia"
    Intent layers were never an official Mycroft feature. They began as a personal experiment in an early **mycroft-core** fork about ten years ago — built on the global enable/disable-intent mechanism and only ever wired into a single demo skill. **OpenVoiceOS** is where the idea was properly modernized: re-implemented on per-session [intent context](context.md) so it behaves correctly under multi-session / **[HiveMind](https://jarbashivemind.github.io/HiveMind-community-docs/)** setups.
