...
> **EXPERIMENTAL** — `ovos-media` is the new standalone audio/video daemon for OpenVoiceOS.
> It is a work in progress and does not yet ship with OVOS by default, but can be manually enabled.
...
The pipeline plugin tracks one `OCPPlayerProxy` per session — important for HiveMind where each
...
`ovos_media/gui.py` still ships Qt5 QML files (`ovos_media/qt5/`) and drives the GUI in the
