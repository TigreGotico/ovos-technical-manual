...
[ovos-shell](https://github.com/OpenVoiceOS/ovos-shell) is the **production Qt5/Kirigami
shell application** for OVOS on embedded and desktop devices (Mark 2, Raspberry Pi with
touchscreen, laptop). It wraps the `mycroft-gui-qt5` library (`Mycroft 1.0` QML module)
...
| `MediaWidgetButton.qml` | OCP playback controls widget |
...
- Wallpaper via `homescreen.wallpaper.set` (PHAL wallpaper manager)
...
`ovos-shell` is tightly coupled to [PHAL](340-PHAL.md). The following PHAL plugins
