...
|-------|------|---------|-------------|
| `version_file` | string | `""` | Path to `version.py` (relative to repo root). **Empty = auto-detect** repo root or `pkg/version.py`. |
| `branch` | string | `dev` | Source branch to checkout and commit the version bump back to |
| `base_branch` | string | `master` | Target branch for the release PR opened by `propose_release` (e.g. `master`/`main`) |
| `publish_prerelease` | boolean | `false` | Create a GitHub pre-release tag after version bump |
| `propose_release` | boolean | `true` | Open a PR from `release-X.Y.ZaN` to `base_branch` |
...
| `publish_pypi` | boolean | `false` | Publish to PyPI after version bump (built inline in this workflow). Requires `PYPI_TOKEN`. |
| `notify_matrix` | boolean | `false` | Send Matrix notification on merged PR. Requires `MATRIX_TOKEN`. |
...
| `matrix_message_pool` | string | `""` | Newline-separated pool of fun prefixes; one is chosen at random and prepended. Empty = skip. |
...
|--------|----------|-------------|
| `PYPI_TOKEN` | when `publish_pypi: true` | PyPI API token for publishing |
| `MATRIX_TOKEN` | when `notify_matrix: true` | Matrix access token for notifications |

Pass these through with `secrets: inherit` (plus the automatic `GITHUB_TOKEN`).
...
| Input | Type | Default | Description |
|-------|------|---------|-------------|
| `version_file` | string | `""` | Path to `version.py` (relative to repo root). **Empty = auto-detect.** |
| `branch` | string | `master` | Stable branch to commit the release to. If `master` is requested but missing, the repo's default branch is used. |
| `publish_release` | boolean | `true` | Create a GitHub release tag |
| `publish_pypi` | boolean | `false` | Publish to PyPI after declaring stable. Requires `PYPI_TOKEN`. |
| `sync_dev` | boolean | `false` | Push `master` → `dev` after stable release to keep branches in sync |
| `notify_matrix` | boolean | `false` | Send Matrix notification on stable release. Requires `MATRIX_TOKEN`. |
...
| `matrix_message_pool` | string | `""` | Newline-separated pool of fun prefixes; one is chosen at random. Empty = skip. |
| `runner` | string | `ubuntu-latest` | Runner label |
...
| `entry_point` | string | `""` | Legacy: a single entry point ID to verify (bypasses auto-detection). Prefer leaving empty (auto-detect) or `entry_points` for multi-plugin wheels. |
| `entry_points` | string | `""` | JSON array of entry point IDs to verify individually, for packages that ship several OPM plugins from one wheel. Each gets its own check. |
...
| `opm_min_version` | string | `""` | Minimum acceptable `ovos-plugin-manager` version; reported as an issue if the installed version is lower |
| `opm_max_version` | string | `""` | Maximum acceptable `ovos-plugin-manager` version |
...
| `NEBULENTO_PIPELINE` | `nebulento` | Add to `[test]` deps |
| `PALAVREADO_PIPELINE` | `palavreado` | Add to `[test]` deps |

Tests that use a missing pipeline are **skipped** (via `is_pipeline_available()`). Use the matching `require_*` input (`require_adapt`/`require_padatious`/`require_m2v`/`require_nebulento`/`require_palavreado`) to fail CI if that pipeline is absent instead of silently skipping; when set, the workflow also auto-installs the plugin as a CI safety net.
...
| `pre_install_pip` | string | `""` | Space-separated pip specs installed before the package (e.g. git URLs for an unreleased sibling) |
| `require_adapt` | boolean | `false` | Fail CI if `ovos-adapt-pipeline-plugin` is not installed (auto-installs as a CI fallback). When `false`, [Adapt](adapt-pipeline.md) tests are skipped if absent. |
| `require_padatious` | boolean | `false` | Fail CI if Padatious is not installed (auto-installs as a CI fallback; adds `swig`). When `false`, [Padatious](padatious-pipeline.md) tests are skipped if absent. |
| `require_m2v` | boolean | `false` | Fail CI if `ovos-m2v-pipeline` is absent (auto-installs as a CI fallback). |
| `require_nebulento` | boolean | `false` | Fail CI if `nebulento` is absent (auto-installs as a CI fallback). |
| `require_palavreado` | boolean | `false` | Fail CI if `palavreado` is absent (auto-installs as a CI fallback). |
| `pre_release` | boolean | `false` | Install `ovoscope` and every required pipeline plugin from its GitHub `dev` branch instead of PyPI — validate against unreleased framework/engine changes. |
| `bus_coverage` | boolean | `true` | Include message-bus coverage in the report |
| `bus_coverage_include` | string | `""` | Regex of bus message types to include in coverage |
| `bus_coverage_exclude` | string | `^Thread-\|^intents$\|^skills$\|^__core__$` | Regex of bus message types to exclude |
...
| `system_deps` | string | `""` | Extra apt packages to install before testing (space-separated) |
| `test_extras` | string | `dev` | pyproject extras key declaring test deps. Installed first via `pip install -e .[<test_extras>]`. |
| `test_extras_fallback` | string | `test` | Extras key tried if `test_extras` is not declared. Empty = skip the fallback. |
| `install_extras` | string | `""` | Extra pip install arguments run before tests, e.g. `.[dev]` or `-r requirements/test.txt`. If empty, the package is installed via the `test_extras` logic above. |
| `pre_install_pip` | string | `""` | Space-separated pip specs installed before the package (e.g. git URLs for an unreleased sibling). |
...
| `deploy_pages` | boolean | `false` | Push the HTML report to `gh_pages_branch`. Replaces `coverage-pages.yml`. Requires `permissions: contents: write`. |
| `gh_pages_branch` | string | `gh-pages` | Branch to push the HTML report to when `deploy_pages: true` |
...
> The workflow declares `permissions: contents: write` so `deploy_pages` can push the report; it is harmless when `deploy_pages: false`.
...
| `ruff_args` | string | `. --exclude _gh_automations` | Args passed to `ruff check`. The default exclude keeps this repo's own checked-out scripts from raising errors in consumer PRs. |
...
| `runner` | string | `ubuntu-latest` | Runner label |
| `python_version` | string | `3.11` | Python version |
| `mypy_args` | string | `.` | Args passed to mypy (e.g. `my_package` or `. --ignore-missing-imports`). Default checks the whole repo. |
| `test_extras` | string | `dev` | pyproject extras key for test/typing deps, installed via `pip install -e .[<key>]` |
| `test_extras_fallback` | string | `test` | Extras key tried if `test_extras` is not declared. Empty = skip. |
| `install_extras` | string | `""` | Extra pip install arguments appended after the package install |
| `system_deps` | string | `""` | Extra apt packages to install before type-checking |
| `fail_on_errors` | boolean | `false` | Fail the job when mypy reports errors |
| `pr_comment` | boolean | `true` | Post the `🔎 Type Check` section on `pull_request` events |
...
|-------|------|---------|-------------|
| `runner` | string | `ubuntu-latest` | Runner label |
| `required_files` | string | `README.md` | Comma-separated list of files that must exist at the repo root (existence check only) |
| `markdownlint` | boolean | `false` | Run `markdownlint-cli2` on `*.md` files. Reported in the PR comment but never blocks merges. |
| `markdownlint_config` | string | `""` | Path to a markdownlint config (e.g. `.markdownlint.json`). Empty = built-in defaults. |
| `fail_on_missing` | boolean | `false` | Fail the job if any required file is absent |
| `pr_comment` | boolean | `true` | Post the `📚 Docs` section on `pull_request` events |
...
| `spec_version` | string | `""` | OVOS spec version to target. Empty = `ovos-spec-lint` default. Set `0`/`1`/`2`/`3` to flag features newer than that version. |
...
| `ovos_spec_tools_spec` | string | `ovos-spec-tools` | pip spec for `ovos-spec-tools` (override to pin/test a branch) |
| `pr_comment` | boolean | `true` | Post a section on `pull_request` events |
...
| `runner` | string | `ubuntu-latest` | Runner label |
| `python_version` | string | `3.11` | Python version |
| `system_deps` | string | `""` | Extra apt packages to install before testing |
| `install_extras` | string | `test` | pip extras to install (must pull in `ovoscope`) |
| `test_path` | string | `test/end2end/` | Path passed to pytest |
| `require_adapt` / `require_padatious` / `require_m2v` / `require_nebulento` / `require_palavreado` | boolean | `false` | Fail CI if the named pipeline is absent (otherwise skipped) |
| `pre_release` | boolean | `false` | Install ovoscope + required pipelines from GitHub `dev` instead of PyPI |
| `langs` | string | `'["all"]'` | JSON array of languages to shard the matrix by |
| `accuracy_baseline` | string | `""` | Path to a baseline accuracy file to compare against |
| `accuracy_tolerant` | boolean | `false` | Treat an accuracy regression as a warning rather than a failure |
| `accuracy_min` | string | `""` | Minimum accuracy threshold to enforce |
| `pr_comment` | boolean | `true` | Post the `🎯 Intent-Case Accuracy` section on `pull_request` events |
...
| `runner` | string | `ubuntu-latest` | Runner label |
| `python_version` | string | `3.11` | Python version |
| `system_deps` | string | `""` | Extra apt packages (e.g. `espeak-ng sox` for engines backed by a system binary) |
| `install_extras` | string | `test` | pip extras to install (must pull in `ovoscope[tts]`) |
| `test_path` | string | `test/end2end/test_tts_intelligibility.py` | Path passed to pytest |
| `max_wer` | string | `0.5` | Maximum acceptable mean WER; the test asserts below this |
| `pre_release` | boolean | `false` | Install `ovoscope[tts]` from GitHub `dev` instead of PyPI |
| `warn_only` | boolean | `false` | Report the score but never fail the job |
| `pr_comment` | boolean | `true` | Post the `🗣️ TTS Intelligibility` section on `pull_request` events |
