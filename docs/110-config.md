...
> Admin PHAL is a special service that runs as root — it can **only** access `/etc/mycroft/mycroft.conf`.
...
- [Bus Service](100-bus_service.md) — `websocket` config section
- [Bus Client](900-bus_client.md) — `websocket` and `session` config sections
- [ovos-core](102-core.md) — `skills`, `intents`, `utterance_transformers` config sections
