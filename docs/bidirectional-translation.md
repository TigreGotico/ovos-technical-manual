...
!!! info "Roadmap"
    The companion `ovos-translate-server-plugin` is gaining a `server_type` option so a device can offload translation to LibreTranslate or DeepL servers, not just an OVOS translate server ([PR #24](https://github.com/OpenVoiceOS/ovos-translate-server-plugin/pull/24)).
